...
npx design-superskills --bundle career -y
```
...
```bash
npx design-superskills --list-bundles
```
...
npx design-superskills --search "research"
```
...
- Claude Cowork updates are manual by design: generate the zip, remove the previous `design-superskills` plugin in Cowork, then upload the new zip.
