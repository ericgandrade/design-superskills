...
function getManagedSkillNames() {
  ...
  const cacheBase = path.join(homeDir, '.design-superskills', 'cache');
...
async function runNuclearFlow(quiet) {
  ...
  console.log(chalk.red('  obsidian-superskills, product-superskills, and any others.\n'));
...
async function runUninstallFlow(detected, quiet, skipPrompt, scope = 'global', projectRoot = process.cwd()) {
  ...
  if (!skipPrompt) {
    ...
    const { clearCacheNow } = await inquirer.prompt([{
      type: 'confirm',
      name: 'clearCacheNow',
      message: 'Clear local skills cache (~/.design-superskills/cache) too?',
      default: true
    }]);
...
async function clearSkillsCache(quiet) {
  const cacheBase = path.join(os.homedir(), '.design-superskills', 'cache');
...
async function main() {
  ...
  console.log(chalk.cyan.bold(`\n🚀 design-superskills v${VERSION} - Multi-Platform Installer\n`));
  ...
  switch (command) {
    ...
    case 'doctor':
      console.log('Use: npx design-superskills --help for options\n');
      break;
...
Usage: npx design-superskills [COMMAND] [OPTIONS]
...
Examples:
  npx design-superskills                        # Interactive smart installation
  npx design-superskills -y -q                 # Auto smart update/install
  npx design-superskills --bundle essential -y # Install essential bundle
  npx design-superskills up -y                 # Update outdated + install missing skills
  npx design-superskills package-cowork        # Generate Cowork zip for manual upload
  npx design-superskills status                # Show global status and version differences
  npx design-superskills uninstall -y          # Uninstall all + clear cache
  npx design-superskills uninstall --nuclear   # ☢️  Remove ALL skills from ALL platforms
  npx design-superskills --local                   # Install to current project
  npx design-superskills --local -y                # Auto install locally
  npx design-superskills update --local            # Update local skills only
  npx design-superskills uninstall --local -y      # Remove local skills
`);
}
