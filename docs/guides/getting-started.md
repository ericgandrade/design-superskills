...
```bash
npx design-superskills ls
```

Or the long form:

```bash
npx design-superskills list
```

### 3. Get Help

```bash
npx design-superskills help
```

Or check a specific command:

```bash
npx design-superskills install --help
```
...
```bash
npx design-superskills --bundle developer -y
```
...
```bash
npx design-superskills --search "prompt"
npx design-superskills --search "video"
npx design-superskills --search "transcription"
```
...
Examples:
```bash
npx design-superskills i -a -y    # Install all with flags
npx design-superskills ls          # Quick list
npx design-superskills up          # Update skills
```
...
Examples:
```bash
npx design-superskills install -a -y -q    # Install everything silently
npx design-superskills list -q             # List without extra formatting
npx design-superskills update -g -y        # Global update with confirmation skip
```
