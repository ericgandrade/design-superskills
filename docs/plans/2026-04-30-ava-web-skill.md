...
Create the `ava-web` skill to generate websites, landing pages, and web components with Avanade visual identity.
...
- **Hero**: orange→purple full-width gradient, white text, bottom SVG wave
- **Nav**: logo on left, links in center/right, orange CTA button
...
- **Step 2: Select components** — map necessary sections to the requested page type
- **Step 3: Generate** — production-ready code, commented per section
...
`evals/trigger-eval.json` — 15 queries (10 true, 5 false like Python scripts, audio, Excel).

### Task 7: Update repository metadata

- `README.md`: 63→64 skills, add `ava-web` to UI/UX Design table
- `CLAUDE.md`: count + skills tree + bundles
- `CHANGELOG.md`: entry v1.25.0
- `bundles.json`: add `ava-web` to `content` bundle (branded content) and `all`
